...
# データベースに天気データを保存する関数
def save_weather_data_to_db(region_code, weather_data):
    ...
    try:
        ...
        for area in areas:
            for date, description in zip(time_series[0]["timeDefines"], area["weathers"]):
                cursor.execute('''
                    INSERT INTO weather_forecast (region_code, date, weather_description, min_temp, max_temp)
                    VALUES (?, ?, ?, ?, ?)
                ''', (region_code, date[:10], description, "N/A", "N/A"))  # 気温データは後で追加可能

        conn.commit()
        print("天気データがデータベースに保存されました！")
...
def fetch_weather_data(region_code):
    try:
        ...
        # 天気データをデータベースに保存
        save_weather_data_to_db(region_code, data)
